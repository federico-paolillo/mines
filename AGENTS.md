...
The frontend project structure is withing the `fe/` folder and follows a default Preact project with Vite and Vitest. Routable pages are in the `pages/` folder. Each page-related component is within a given page `components/` folder. For example: Home page only components are under `pages/Home/components/`. General, shareable components, are in the `components/` top-level folder. The folder `client/` is automatically generated by using the Kiota tool that will consume the Open API specification produced by the Swag tool.
